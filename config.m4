dnl config.m4 for sapi fpmi

PHP_ARG_ENABLE([fpmi],,
  [AS_HELP_STRING([--enable-fpmi],
    [Enable building of the fpmi SAPI executable])],
  [no],
  [no])
...
if test "$PHP_FPMI" != "no"; then
  ...
  PHP_ARG_WITH([fpmi-user],,
    [AS_HELP_STRING([[--with-fpmi-user[=USER]]],
      [Set the user for php-fpmi to run as. (default: nobody)])],
    [nobody],
    [no])

  PHP_ARG_WITH([fpmi-group],,
    [AS_HELP_STRING([[--with-fpmi-group[=GRP]]],
      [Set the group for php-fpmi to run as. For a system user, this should
      usually be set to match the fpmi username (default: nobody)])],
    [nobody],
    [no])

  PHP_ARG_WITH([fpmi-systemd],,
    [AS_HELP_STRING([--with-fpmi-systemd],
      [Activate systemd integration])],
    [no],
    [no])

  PHP_ARG_WITH([fpmi-acl],,
    [AS_HELP_STRING([--with-fpmi-acl],
